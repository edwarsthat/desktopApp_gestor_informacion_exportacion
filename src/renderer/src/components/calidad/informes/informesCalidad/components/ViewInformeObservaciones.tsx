...
export default function ViewInformeObservaciones(props: propsType): JSX.Element {
    if (props.loteSeleccionado.calidad?.clasificacionCalidad === undefined) {
        return <div>Lote sin clasificación descarte...</div>
    }
    ...
    const seleccionarObservaciones = (): void => {
        ...
        if (props.loteSeleccionado.calidad && props.loteSeleccionado.calidad?.clasificacionCalidad) {
            Object.entries(props.loteSeleccionado.calidad.clasificacionCalidad).forEach(
                ([key, value]) => {
                    ...
                    if (typeof value === 'number') {
                        ...
                        console.warn(
                            `descarte.descarteGeneral is undefined or not a number. Skipping calculation for key ${key}.`
                        )
                    }
                }
            )
        ...
        <div>
            <div className="informe-calida-lote-div-calidad-interna">
                {props.loteSeleccionado &&
                    props.loteSeleccionado.calidad &&
                    props.loteSeleccionado.calidad?.calidadInterna &&
                                            ...
                                            {props.loteSeleccionado.calidad?.calidadInterna?.peso !== 0
                                                ? (
                                                    (props.loteSeleccionado.calidad?.calidadInterna.zumo /
                                                        props.loteSeleccionado.calidad?.calidadInterna?.peso) *
                                                    100
                                                ).toFixed(2) + '%'
                                                : 'NaN'}
                                        </p>
            ...
            <div className="informe-calida-lote-div-observaciones">
                {dataObservaciones &&
                    observaciones.map((item) => <p key={item}>{dataObservaciones[item]}</p>)}
            </div>
        </div>
    )
}
